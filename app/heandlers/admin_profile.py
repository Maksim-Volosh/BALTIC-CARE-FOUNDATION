...
from app.heandlers.user_profile import user_profile
from app.states import AddPlace, AdminPlace, ChoiceUser
from app.utils.utils import is_admin, paginate_users, validate_place
...
async def delete_yes(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text(f'Статистика @{username} удалена!')
...
async def tokens(callback: CallbackQuery):
    """
    Show user tokens.
    """
    db = Database(config.DB_NAME)
    if await is_admin(callback):
        ...
        message_text = f'Ваши токены:\n\n'
        if tokens:
            for token in tokens:
                message_text += f'{token[1]}\n'
        else:
            message_text += 'Токенов нет. Добавьте новый!'
        await callback.message.edit_text(message_text, reply_markup=prkb.admin_tokens)
    else:
        await callback.message.edit_text('Вы не администратор!', reply_markup=prkb.pback)
        

@router.callback_query(F.data == 'add_token')
async def add_token(callback: CallbackQuery):
    """
    Add token???
    """
    await callback.message.edit_text('Вы уверены что хотите добавить новый токен?', reply_markup=prkb.admin_add_token)


@router.callback_query(F.data == 'add_token_yes')
async def add_token_yes(callback: CallbackQuery):
    """
    Add token yes.
    """
    db = Database(config.DB_NAME)
    db.add_token()
    await tokens(callback)